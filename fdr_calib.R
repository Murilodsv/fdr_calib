...
# Key variables:
# rswc_sf  - fraction of level for swc in pmp and sat in respect to Scaled Frequency (SF) 
# (i. e ): If all rswc_sf = 1, the calibration assumes that the minimum SF ~ Wilting Point (15000 Kpa) and the maximum SF ~ Saturation Point (10 Kpa)
# ret_curv - retention curves data determined on the experimental area

# History of events:
# - Folegatti's Diviner 2000 was used on ratoon 1 and 2 seasons.
# - A new Diviner 2000 acquired by GEPEMA was used for season 3
# - Unfortunately, the Folegatti's Diviner broke down and no comparable measurements could be done
# - Therefore, this code was developed to calibrate separately the Folegattis and GEPEMA Diviners 2000
# - Calibration is assuming that 
# - 1) Soil Water Content (SWC) can be computed from the Scaled Frenquencies (SF) based on the manufacturer equation (swc = 10 ^ (log10((sf-c)/a)/b))
# - 2) The parameters abc can be calibrated to fit SF to retention curves measurements [c(min(SF),max(SF)) ~ c(min(SWC),max(SWC))]


#--- IMPORTANT NOTES:
#---   - SWC for Folgatti diviner equipment were already calibrated for depths 10-80cm. Here we calibrate the depth > 80cm
#---   - SWC for GEPEMA diviner equipment were re-calibrated for all depths 10-150cm
  ...
  if(opt_ly){
    ...
    for(tb in l_tb){
      ...
      for(dp in l_dp){
        ...
        message(paste("Calibrating tube",tb,"at",dp,"cm depth (GEPEMA Diviner 2000)"))
...
if(opt_fol){
  ...
  #--------------------------------------------------
  #--- calibrate only for bottom layer (dp > 80) ----
  #--------------------------------------------------
  #--- the layers 10-80 were already calibrated!!!
  
  fdr_opt = fdr_opt[fdr_opt$depth > 80,]
  ...
  if(opt_ly){
    ...
    for(tb in l_tb){
      ...
      for(dp in l_dp){
        ...
        message(paste("Calibrating tube",tb,"at",dp,"cm depth (Folegatti Diviner 2000)"))
    ...
    #--- update calibrated swc for dp>80cm
    fdr_fol = rbind(fdr_fol[fdr_fol$depth <= 80,],fdr_opt_ly)
  }else{
...
#--- update both folegatti and gepemas equipment caliration data
...
if(plot_adc){
  ...
  # WARNING THIS FOLLOWING CODE WAS NOT TOTALLY TESTED (DEBUGGIN PURPOSE)
  # KEEP plot_adc = F
